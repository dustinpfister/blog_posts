...
This is a post on the on blur event in client side [javaScript](/2018/11/27/js-getting-started/), one of many events to which an event handler can be attached for an element. An on blur event can be attached to an element using the on blur property of an element, or a method like addEventListener to attach event handers that will fire when the blur event happens. 

I will not be going over [event listeners](/2019/01/16/js-event-listeners/) in general, as well as all the various [details of an event object](/2020/07/23/js-event-object/) in this post as I have wrote a posts on those subjects in the past. So then this post will be focusing on mainly the on blur event and related topics such as the tab index property. I assume that you have at least some background with javaScript and HTML as this is required in order to get anything of value from this post.
