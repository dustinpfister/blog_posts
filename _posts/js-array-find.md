...
If I want to get not just one element, but all elements that meet a condition I can use the [array filter](/2020/10/03/js-array-filter/) method. This will create and return a new array where each element is an element that meets the conditon that was given with a callback rather than just the first match.
