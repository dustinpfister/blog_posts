...
There are many methods that can be used with arrays that are in the array prototype that can be used to preform all kinds of typical tasks with arrays, but there is not every little methods that comes to mind, so in some cases one might want to use some kind of additional library on top of what is in the native prototype object such as [lodash](/2019/02/15/lodash/). These native array prototype methods help with sorting, filtering, and mapping them in all kinds of different ways depending in additional logic that is used with them. Many of these methods have been part of the javaScript spec for a long time and are thus very safe to use when it comes to concerns of engine support. Others are a little newer so there should be a degree of concern at least maybe depending on the situation with platform support.
...
One way to create an array is to use the Array constructor with the [new keyword](/2019/02/08/js-javascript-new/) to create a new empty array of a set length. This is used more or less the same way as any other [constructor function](/2019/02/27/js-javascript-constructor/) where you type the new operator followed by calling the constructor function to create an instance of that constructor which in this case would be an Array. There are many other built in constructors in native javaScript such as Date, String, and Object just to name a few. There is also how to go about creating user defined constructor functions, and so forth, but I do not care to get to far off topic with that in the post at least.
...
I do not recommend the use of the Array constructor, and generally prefer using the literal syntax in place of this. There are also just so many other great options for creating an array, or ending up with one from all kinds of various sources. However the array constructor does show up in many code examples on the open web so it is something that a javaScript developer should be aware of at least.
...
There is a great deal more to write about when it comes to javaScript Arrays, that should go without saying as there is just so much that revolves around them, as well as with collection in general of course. I did not even scratch the surface when it comes to every little thing to know about with Arrays in javaScript. For example there is more to write about when it comes to typed arrays, and how they differ from the regular typical arrays that are used in javaScript. In [lodash there is a lengthly collection of methods that can be used to help with common programming tasks surrounding arrays](/2019/02/14/lodash_array/), as well as with objects in general to discus as well. Hopefully this post did a decent job of covering some of the basics and then some though.