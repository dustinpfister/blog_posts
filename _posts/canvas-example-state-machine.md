...
May frameworks such as phaser will have a state machine as part of the functionality of the framework. When it comes to making a canvas framework I often thing that a state machine should be a part of such a framework, but I guess it does not have to be when it comes to making such a framework. In any case this post will be on just one way to go about making a state machine by itself without much more beyond that.
