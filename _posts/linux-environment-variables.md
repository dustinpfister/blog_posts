...
First off I think that it should go without saying that you should not overwrite any important variables in the the environment when exporting Shell variables. So just use the printenv command to make sure that any name that is set for a variable name is not taken just yet. In addition if you really want to change an environment variable value, just make sure that you know what it is used for. The changes will not be permanent unless you make the necessary chances in an etc file, more on that later.
