...
## 1 - What to know before getting started with javaScripts

Learning javaScript as with any programing language takes a whole lot of time and patience, as well as dedication as there is a whole lot of ground to cover and one is just not going to become aware of all of it in just one day. There is just way to much to write about when it comes to the idea of trying to wrap up everything into a single blog post. 
...
In this post I am using Google chrome 96.x, which is important when using examples that are to be copied and pasted into the javaScript console. Also using an up to date browser is important when it comes to testing out just about anything that has to do with client side javaScrit anyway. If by chance you end up working with some javaScript code that makes use of a very later browser feature that code might break if you are trying to get it to work in an old web browser vendor and version that is no longer supported.
