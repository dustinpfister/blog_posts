...
When it comes to setting the handler I will often want to use the [on ready state method](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/onreadystatechange) to do so. It is in here that I want to check to make sure that the ready state is 4, and that the status of the request is 200. In more advanced examples I might want to check for a given range when it comes to status codes, and will also want to do something when I have a status code that indicates that something went wrong, but for now lets keep things simple.
