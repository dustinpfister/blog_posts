...
This is a post on an advanced topic of threejs that has to do with the position attributes of buffer geometry instances, and one way to go about transitioning between the two. That is one way not the way as there is a whole lot of other things to write about when it comes to covering some ground with animation in threejs. 
