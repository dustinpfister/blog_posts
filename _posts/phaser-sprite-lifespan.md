...
In this post I was using phaser community edition 2.11.0 of [phaser](https://phaser.io/).

## 2 - An example of Sprite.lifespan

For an example of Sprite.lifespan I put together a quick example that involves a button sprite that will result in a bunch of sprites springing from the single button sprite when clicked. The sprites will have random lifespans and will spring up and out and fall down by way of gravity. Also as the lifespan of a sprite runs out, and [alpha transparency](/2018/09/18/phaser-sprite-alpha/) effect will also be in effect on the sprites as then reach death. Once the lifespan time runs out the sprites will be killed automatically by phaser, but will still exist in a group I made from them, ready to be revived once again.
...
So the Sprite.lifespan property is great for setting a time limit for how long a sprite will remain in the game until it just dies out to end up ready to be brought back into the game when needed with sprite.revive. This can be used with or as a complete replacement for the [Sprite.health property](/2018/09/17/phaser-sprite-health/), and Sprite.damage method that are also must know methods for handling the death of sprites.

Thank you for reading if you would like to see more examples on this, or have any questions or concerns be sure to let me know in the comments section.