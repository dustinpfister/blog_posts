...
There are other ways to break a loop as well, such as using the [return keyword](/2019/03/01/js-javascript-return/) within the body of a function for example. There is another keyword that is similar to that of break called the [continue keyword](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/continue) that is worth mentioning also as that keyword can be used to skip a body of code and continue a loop without breaking out of it, but just skipping over any additional code that would run otherwise. 
...
For this example I am using a switch in the body of a function that I ten intend to use with the map array prototype method.
